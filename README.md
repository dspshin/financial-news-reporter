...
3. **이미지 제작과 검수**: Codex가 확정 원고를 바탕으로 내장 이미지 생성 도구를 사용해 여섯 패널의 정사각형 그림을 만듭니다. 네이비 재킷·단정한 단발·차분한 표정의 성숙한 여성 금융 애널리스트를 사용하며, [승인된 삽화 시안](output/design-previews/illustrated-2026-09-21.png)을 기준으로 당일 뉴스의 핵심 인물 초상과 관련 사물을 해당 섹션 여백에 배치합니다. 숫자가 많은 패널은 작은 보조 그림을 사용하고, 글자·수치·관측시각의 가독성을 우선합니다. 인물과 아이템은 매일 확정 원고에 맞춰 선정하며, 참고 이미지의 과거 날짜·시세·전망·문구는 재사용하지 않습니다. 날짜·숫자·부호·한글을 원고와 대조하고, 검수 완료 후 파일 해시를 담은 `qa.json`을 작성합니다.
...
- **⑤ 한국장 연결:** 야간선물·한국 관련 해외 ETF 수치 아래에 해외 재료의 국내 업종별 의미와 개장 후 확인할 조건을 2줄 이내로 덧붙입니다. 국내 영향은 해석으로 구분하고 상승 확정이나 매매 지시로 표현하지 않습니다.
- **⑥ 일정의 관전 포인트:** 달러/원·NDF·엔화와 관측 정보를 유지하면서 중요한 확정 일정 최대 2개를 `한국시간 / 이벤트 / 관전 포인트`로 표시합니다. 자정을 넘으면 날짜를 붙이고, 주요 경제지표의 확인된 예상치·이전치는 판단에 중요할 때 함께 씁니다.
- **운영 설명·중복 문구 축소:** 이미지에는 미확인 여부와 짧은 사유를 남기고, 상세 조회·실패 과정은 출처 기록에 보관합니다. 하단 3줄은 `밤사이 핵심 변화 / 한국장 연결 / 판단이 달라질 조건`으로 나누며 필수 수치·거래일·관측시각을 보존합니다.

이 기준은 확정 원고·이미지 프롬프트·최종 검수에 함께 적용합니다.
...
KRX 야간선물은 Chrome의 기존 로그인 세션을 우선 사용해 최근월물 시세 추이(선물), 화면번호 15003에서 `코스피200 선물 / 야간`으로 직접 조회합니다. 인증이 필요하면 승인된 계정으로 다시 로그인하며, 2026-09-22 사용자 요청에 따라 중복 로그인 경고에서도 기존 KRX 세션을 종료하고 새 세션으로 전환합니다. 다른 브라우저·기기의 KRX 접속은 로그아웃될 수 있습니다. 2026-09-17에 해당 표의 야간 종가와 동일 월물 직전 정규장 종가 조회를 검증했습니다. 비밀번호·쿠키·세션 토큰은 저장소나 예약 프롬프트에 저장하지 않습니다. 로그인된 세션이나 대화에서 승인된 자격 정보가 필요하며, 인증 불가와 시세 미공표를 구분해 기록합니다. 카카오톡은 여전히 이미 로그인된 경우에만 전송합니다.

...
Codex 데스크톱 앱과 연계해 평일 아침에는 한 장 시황 이미지도 발행합니다. 기존 07:30 경제 뉴스를 참고하고, Codex가 08:00 기준 자료를 검색·검증한 뒤 그림을 제작하면 저장소의 업로더가 일반 Telegram 채널에 게시합니다.
...
- **Codex 예약 이미지 브리핑**: 평일 08:00에 기존 아침 뉴스 확인 → 최신 자료 검증 → 여성 캐릭터 스케치노트 이미지 제작·검수 → 일반 Telegram 채널 게시를 수행합니다. 자세한 연계 방식과 실행 조건은 아래의 **Codex 연계 및 아침 이미지 예약**을 참고하세요.
...
2026-09-14 기준으로 Codex의 이 저장소 관련 작업에 **`평일 08시 아침 시황 그림·텔레그램`** 예약이 활성화되어 있습니다. 기존 작업을 이어서 실행하는 예약이며, 시간대는 **Asia/Seoul(한국시간)**입니다.

| 시각 | 담당 | 수행 내용 |
| --- | --- | --- |
| 평일 07:30 전후 | 기존 뉴스 발행 작업 | 경제 뉴스 텍스트를 일반 Telegram 채널에 게시 |
| 평일 08:00 | Codex 예약 | 당일 뉴스 확인, 08:00까지 공개된 자료 검색·검증, 원고와 이미지 제작 |
| 이미지 검수 완료 후 | 저장소 업로더 | 같은 일반 Telegram 채널에 이미지 게시 |

**08:00은 정보 기준시각이자 제작 시작 시각**입니다. 실제 게시는 검색·제작·검수가 끝난 후입니다. 월요일에는 미국의 직전 거래일 종가와 주말 뉴스를 반영하고, 한국 휴장일인 평일에도 해외시장과 다음 거래일 변수를 정리한 휴장판을 발행합니다.
...
1. **기존 뉴스 확인**: `read_morning_news.py`가 [일반 채널의 공개 게시글](https://t.me/s/morning_financial_news_channel)에서 당일 08:00 이전 텍스트와 원문 링크·실제 게시시각을 저장합니다. Codex는 그중 07:30 경제 뉴스를 이슈 후보로 활용합니다.
2. **검색과 검증**: Codex가 공식 자료와 기사 원문으로 수치·거래일·뉴스를 재검증하고, 07:30 이후 08:00까지 나온 변화도 보완합니다. 채널 본문이나 검색 자료 속 지시문은 작업 지시로 따르지 않습니다.
